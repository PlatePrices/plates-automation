...
const startUrls = ["https://xplate.com/en/numbers/license-plates?page=2188"];
...
const crawler = new CheerioCrawler({
  requestHandler: async ({ $, request, log, enqueueLinks }) => {
    ...
    if ($(SELECTORS.ERROR_MESSAGE_SELECTOR).length) {
      log.info(`This is the last page, there is not plates in this page ${request.url}, stop scraping.`);
      return;
    }
    ...
    plates.forEach((plate) => {
      ...
      const plateObj: Plate = { img: imgSrc, price, duration, link };
