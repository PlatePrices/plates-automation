...
export const CONFIG = (pageNumber: number) => {
  const data = JSON.stringify({
    requests: [
      {
        indexName: 'motors.com',
        query: '',
        params: `page=${pageNumber.toString()}&attributesToHighlight=[]&hitsPerPage=25&attributesToRetrieve=["is_premium","is_featured_agent","location_list","objectID","name","price","neighbourhood","agent_logo","can_chat","has_whatsapp_number","details","photo_thumbnails","photos","highlighted_ad","absolute_url","id","category_id","uuid","category","has_phone_number","category_v2","photos_count","created_at","site","permalink","has_vin","auto_agent_id","is_trusted_seller","show_trusted_seller_logo","trusted_seller_logo","trusted_seller_id","created_at","added","jobs_logo","vas","seller_type","is_verified_user","has_video","cotd_on","is_super_ad","categories","city","bedrooms","bathrooms","size","neighborhoods","agent","room_type","is_reserved","is_coming_soon","inventory_type"]&filters=("category_v2.slug_paths":"motors/number-plates") AND ("site.id":"2" OR "multi_site":true)`,
      },
    ],
  });
  return {
    method: DUBIZZLE_SELECTORS.CONFIG.method,
    headers: {
      ...DUBIZZLE_SELECTORS.CONFIG.headers.HEADERS,
      'Content-Type': 'application/json',
    },
    body: data,
  };
