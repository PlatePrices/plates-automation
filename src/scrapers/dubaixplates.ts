...
const fetchDubaiXplatesPage = async (pageNumber: number): Promise<void> => {
  ...
  for (const plate of plates) {
    ...
    if (plateElement.find(DUBAI_XPLATES_SELECTORS.PLATE_EXCEPTION_SELECTOR)) continue;
    const number = plateElement.find(DUBAI_XPLATES_SELECTORS.PLATE_NUMBER).text().trim();
    const price = plateElement.find(DUBAI_XPLATES_SELECTORS.PLATE_PRICE).text().trim();
    const emirate = plateElement.find(DUBAI_XPLATES_SELECTORS.PLATE_SOUCE).attr('src');
    const character = plateElement.find(DUBAI_XPLATES_SELECTORS.PLATE_CHARACTER).text().trim();
    const url =
      DUBAI_XPLATES_SELECTORS.SHARABLE_LINK + plateElement.find(DUBAI_XPLATES_SELECTORS.PLATE_URL).attr('href');
